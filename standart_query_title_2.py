...
def query_with_subtitle(col_name1, col_name):
    get_col_name_8= """SELECT %s FROM test WHERE id_count=2 GROUP BY %s """ % (col_name1, col_name1)
    cursor.execute(get_col_name_8)
    col_name_8 = [item for t in cursor.fetchall() for item in t]
    print(col_name_8)

    get_col_name_9 = """SELECT %s FROM test WHERE id_count=3 GROUP BY %s """ % (col_name, col_name)
